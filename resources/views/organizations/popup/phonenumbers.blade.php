...
<form class="ajax-Form" enctype="multipart/form-data" method="post" action="{!! URL::to('/organization/unassignedupdate') !!}">
    ...
    <div class="row">
        <div class="col-md-12">
            <label for="exampleInputEmail1">Unassigned Phone Numbers</label>
            <div class="m-b-30">
                <select name="grplist" class="form-control" id="multiselect">
                    <option value="">Select</option>
                    @foreach($pPhoneNumbers as $pPhoneNumber)
                        @if(!empty(trim($pPhoneNumber->extension)))
                            <option
                                    value="{{$pPhoneNumber->extension}}::{{\App\Helpers\Helper::format_phonenumber($pPhoneNumber->extension)}}::{{$pPhoneNumber->description}}"
                            >
                                {{ \App\Helpers\Helper::format_phonenumber($pPhoneNumber->extension) }} - {{$pPhoneNumber->description}}
                            </option>
                        @endif
                    @endforeach
                </select>
    ...
    <div class="form-actions pull-right depend">
        <input type="hidden" name="type" value="phonenumbers">
